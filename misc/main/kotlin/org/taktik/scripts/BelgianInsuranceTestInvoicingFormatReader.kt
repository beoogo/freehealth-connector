...
import org.taktik.freehealth.middleware.format.efact.Record
...
class BelgianInsuranceTestInvoicingFormatReader(private val language: String) {

    private var errorCodes: List<EfactError>? = null

    private fun getErrorCodeDescription(errorCode: String): String? {
        if (this.errorCodes == null) {
            this.errorCodes = Gson().fromJson<List<EfactError>>(this.javaClass.getResourceAsStream("/be/errors/EfactErrors.json")
                ?.reader(Charsets.UTF_8) ?: throw FileNotFoundException("EfactErrors.json"), object : TypeToken<ArrayList<EfactError>>() {}.type)
        }
        return errorCodes?.find { it.code == errorCode }?.let { c -> c.label[language]?.let { label -> "${c.type}:$label" } }
    }
    ...
    private fun readErrorDetails(reader: ReaderSession, record: Record<*>) = ErrorDetail().apply {
        ...
        this.rejectionCode1?.let {
            if (it != "000000" && "BREFS".contains(this.rejectionLetter1!!)) {
                this.rejectionDescr1 = getErrorCodeDescription(it)
        ...
        this.rejectionCode2?.let {
            if (it != "000000" && "BREFS".contains(this.rejectionLetter2!!)) {
                this.rejectionDescr2 = getErrorCodeDescription(it)
                val zoneDescription = record.description!!.zoneDescriptionsByZone[it.substring(2, 4)] ?: record.description!!.zoneDescriptionsByZone[it.substring(2, 4)+"a"]
        ...
        this.rejectionCode3?.let {
            if (it != "000000" && "BREFS".contains(this.rejectionLetter3!!)) {
                this.rejectionDescr3 = getErrorCodeDescription(it)
                val zoneDescription = record.description!!.zoneDescriptionsByZone[it.substring(2, 4)] ?: record.description!!.zoneDescriptionsByZone[it.substring(2, 4)+"a"]
